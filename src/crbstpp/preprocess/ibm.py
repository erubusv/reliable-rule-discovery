...
def preprocess_ibm(
    ...
    *,
    ...
    frame = pd.read_csv(raw_zip, usecols=usecols)
    frame = frame.rename(
        columns={
            "From Bank": "from_bank",
    ...
    frame["time"] = ((frame["timestamp"] - origin).dt.total_seconds() // 60).astype(
        np.int64
    )
    frame["hour"] = frame["time"] // 60
    ...
    hourly_out = (
        calibration.groupby(["from_code", "hour"], sort=False).size().to_numpy()
    )
    hourly_in = calibration.groupby(["to_code", "hour"], sort=False).size().to_numpy()
    ...
    paid = (
        pd.to_numeric(frame["paid"], errors="coerce")
        .fillna(0)
        .to_numpy(dtype=np.float64)
    )
    received = (
        pd.to_numeric(frame["received"], errors="coerce")
        .fillna(0)
        .to_numpy(dtype=np.float64)
    )
    ...
    prior_out_mean = np.divide(
        prior_out_sum, out_count, out=np.zeros_like(paid), where=out_count > 0
    )
    prior_in_mean = np.divide(
        prior_in_sum, in_count, out=np.zeros_like(received), where=in_count > 0
    )
    out_pair_count = (
        frame.groupby(["from_code", "to_code"], sort=False).cumcount().to_numpy()
    )
    in_pair_count = (
        frame.groupby(["to_code", "from_code"], sort=False).cumcount().to_numpy()
    )
    ...
    previous_pair_out_time = (
        frame.groupby(["from_code", "to_code"], sort=False)["time"]
        .shift()
        .to_numpy(dtype=np.float64)
    )
    previous_pair_in_time = (
        frame.groupby(["to_code", "from_code"], sort=False)["time"]
        .shift()
        .to_numpy(dtype=np.float64)
    )
    previous_out_time = out_group["time"].shift().to_numpy(dtype=np.float64)
    previous_out_gap = (
        pd.Series(frame["time"].to_numpy() - previous_out_time)
        .groupby(frame["from_code"], sort=False)
        .shift()
        .to_numpy(dtype=np.float64)
    )
    ...
    for index, row in enumerate(
        frame[["from_code", "to_code", "time"]].itertuples(index=False)
    ):
        last_in = last_incoming_by_account.get(int(row.from_code))
        if last_in is not None and (
            not np.isfinite(previous_out_time[index])
            or previous_out_time[index] <= last_in
        ):
            turnaround[index] = True
        last_incoming_by_account[int(row.to_code)] = int(row.time)
    out_hour_pos = (
        frame.groupby(["from_code", "hour"], sort=False).cumcount().to_numpy() + 1
    )
    in_hour_pos = (
        frame.groupby(["to_code", "hour"], sort=False).cumcount().to_numpy() + 1
    )
    dormancy = _quantile(
        (
            calibration["time"]
            - calibration.groupby("from_code", sort=False)["time"].shift()
        ).to_numpy(dtype=np.float64),
        0.95,
        1.0,
    )
    ...
    q95_out = _quantile(
        ratio_out[(out_count >= 3) & (frame["timestamp"] <= calibration_end)], 0.95, 5.0
    )
    q05_out = _quantile(
        ratio_out[(out_count >= 3) & (frame["timestamp"] <= calibration_end)], 0.05, 0.2
    )
    q95_in = _quantile(
        ratio_in[(in_count >= 3) & (frame["timestamp"] <= calibration_end)], 0.95, 5.0
    )
    cadence = np.divide(
        out_gap,
        previous_out_gap,
        out=np.full(len(frame), np.nan),
        where=previous_out_gap > 0,
    )
    cadence_q05 = _quantile(cadence[frame["timestamp"] <= calibration_end], 0.05, 0.25)
    return_out_gap = _quantile(
        (frame["time"].to_numpy() - previous_pair_out_time)[
            frame["timestamp"] <= calibration_end
        ],
        0.95,
        1.0,
    )
    return_in_gap = _quantile(
        (frame["time"].to_numpy() - previous_pair_in_time)[
            frame["timestamp"] <= calibration_end
        ],
        0.95,
        1.0,
    )
    out_hour_threshold = max(
        2, int(np.ceil(_quantile(hourly_out.astype(float), 0.95, 3.0)))
    )
    in_hour_threshold = max(
        2, int(np.ceil(_quantile(hourly_in.astype(float), 0.95, 3.0)))
    )
    masks = (
        (out_count >= 3) & (ratio_out >= q95_out),
        (out_pair_count == 0)
        & (out_count >= 1)
        & (frame["from_code"].to_numpy() != frame["to_code"].to_numpy()),
        (in_pair_count == 0)
        & (in_count >= 1)
        & (frame["from_code"].to_numpy() != frame["to_code"].to_numpy()),
        ...
        .ne(out_group["payment_currency"].shift())
        .fillna(False)
        .to_numpy(),
        (out_count >= 1)
        & frame["payment_format"]
        .ne(out_group["payment_format"].shift())
        .fillna(False)
        .to_numpy(),
        (out_count >= 1)
        & frame["to_bank"].ne(out_group["to_bank"].shift()).fillna(False).to_numpy(),
        ...
        & (previous_receiver != frame["to_code"].to_numpy())
        & ((frame["time"].to_numpy() - previous_pair_out_time) >= return_out_gap),
        (in_pair_count > 0)
        & np.isfinite(previous_sender)
        & (previous_sender != frame["from_code"].to_numpy())
        & ((frame["time"].to_numpy() - previous_pair_in_time) >= return_in_gap),
        (out_gap > 0) & (previous_out_gap > 0) & (cadence <= cadence_q05),
    )
    event_parts = []
    for code, mask in enumerate(masks):
        owners = (
            frame["to_code"].to_numpy()
            if code in {2, 4, 11, 13}
            else frame["from_code"].to_numpy()
        )
        event_parts.append(
            pd.DataFrame(
                {
    ...
    entities = pd.DataFrame(
        {
            ...
            "start_time": bounds.reindex(np.arange(len(account_ids)))["min"].to_numpy(
                dtype=np.int64
            ),
            "end_time": bounds.reindex(np.arange(len(account_ids)))["max"].to_numpy(
                dtype=np.int64
            ),
